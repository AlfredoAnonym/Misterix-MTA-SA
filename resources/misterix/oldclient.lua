...
local myMissionLeader = nil
local destBlip, destMarker, destColShape = nil, nil, nil
local missionEntMarker, missionExitMarker = nil, nil
local activeIntData = nil
local missionPassedTick = 0 
local missionFailedTick = 0 
local missionPassedReward = 0 
local activeMusicElement = nil 

local mythX, mythY, mythZ = 0, 0, 0
local isMysteryActive = false
local missionFailedReasonStr = ""
local creatorWnd = nil -- [NEW] Upgrades the variable scope so the whole file can read it

local startX, startY, startZ = 423.250, 2536.497, 16.148
local baseMarker = createMarker(startX, startY, startZ - 1, "cylinder", 2.0, 255, 0, 0, 150)
local baseBlip = createBlip(startX, startY, startZ, 37)
...
addEventHandler("onClientMarkerHit", baseMarker, function(hitElement, matchingDimension)
    if hitElement == localPlayer and matchingDimension and not isPedInVehicle(localPlayer) then
        ...
        -- [NEW] Do nothing if the F4 Creator window is already open
...
addEventHandler("onClientGUIClick", missionList, function()
    ...
    if row ~= -1 then
        ...
        if pData and pData[1] then
            ...
            if v == "Normal" then
                table.insert(killableBy, "Normal")
            else
...
addCommandHandler("invite", function()
    if activeMission then
        return outputChatBox("You cannot accept an invite while in an active mystery.", 255, 0, 0)
    end
...
local musicOptions = {"None", "xfiles1.mp3", "xfiles2.mp3", "myth2.mp3", "myth3.mp3", "myth4.mp3"}
...
addCommandHandler("cm", function()
    if activeMission then
        outputChatBox("You cannot use the Creator Menu while in an active mystery.", 255, 0, 0)
        return
    end
...
bindKey("F4", "down", function()
    ...
    -- [NEW] Block F4 entirely if the Mission Browser is open
    if guiGetVisible(browserWnd) then return end 
    
    if guiGetVisible(creatorWnd) then
        toggleCreatorWindow()
    else
        triggerServerEvent("requestCMOpen", localPlayer)
    end
...
addEventHandler("onClientGUIClick", btnEdit, function(button, state)
    ...
    if row ~= -1 then
        ...
        for i, m in ipairs(musicOptions) do
            if data.music == m then selectedMusicIdx = i - 1 end
        end
...
addEventHandler("onClientGUIClick", btnAddPed, function(button, state)
    ...
    if eType == "Car" then
        local validCar = true
        if modelID == 539 or modelID == 476 then 
            validCar = true
        else
            local vType = getVehicleType(modelID)
            if not vType or vType == "Plane" or vType == "Helicopter" or vType == "Train" or vType == "Trailer" or vType == "Boat" then
                validCar = false
            end
        end
...
addEventHandler("onClientGUIClick", btnRemovePed, function(button, state)
    ...
    if #pedsToSave > 0 then
        ...
        outputChatBox("No entities to remove!", 255, 0, 0)
    end
end, false)
...
addEventHandler("onClientGUIClick", btnSaveMystery, function(button, state)
    ...
    if editingMissionId then
        triggerServerEvent("updateMystery", localPlayer, editingMissionId, name, desc, reward, weather, timeStr, music, pedsJSON, intJSON)
    else
        triggerServerEvent("saveNewMystery", localPlayer, name, desc, reward, weather, timeStr, music, pedsJSON, intJSON)
    end
...
addEventHandler("onClientSetupMission", root, function(missionData, leader)
    ...
    setElementDimension(baseBlip, 9999)
    setElementDimension(baseMarker, 9999)
    
    local pedsData = fromJSON(activeMission.peds)
    while pedsData and type(pedsData) == "table" and #pedsData == 1 and type(pedsData[1]) == "table" and not pedsData[1].model do
        pedsData = pedsData[1]
    end
    ...
    if pedsData and #pedsData > 0 then
        ...
        if iData and iData.entMark then
            ...
            if not isElement(missionEntMarker) then
                ...
                setElementDimension(missionEntMarker, iData.entMark.dim or 0)
            end
            if iData.exitMark and not isElement(missionExitMarker) then
                ...
                setElementDimension(missionExitMarker, iData.exitMark.dim or 0)
            end
            ...
            targetDim = iData.entTarg and iData.entTarg.dim or 0
        else
            targetX, targetY, targetZ = firstPed.x, firstPed.y, firstPed.z
            targetInt = firstPed.interior or 0
        ...
        setElementDimension(destMarker, targetDim)
        setElementInterior(destColShape, targetInt)
        setElementDimension(destColShape, targetDim)
        
        outputChatBox("Mystery Accepted! Head to the location on your radar.", 255, 255, 0)
        
        -- [NEW] Wait 1.5 seconds before listening for collisions to ignore MTA's phantom warp-hits
        setTimer(function()
    ...
    end -- Closes: if pedsData and #pedsData > 0 then
end) -- Closes: addEventHandler("onClientSetupMission", ...)
...
addEventHandler("onClientHideColShape", root, function(weatherID, intDataJSON)
    ...
    if activeMission and activeMission.time and activeMission.time ~= "-1" then
        ...
        if string.find(timeStr, ":") then
            ...
            h = math.floor(timeNum)
            m = math.floor((timeNum - h) * 100)
        end
...
addEventHandler("onClientMarkerHit", root, function(hitElement, matchingDimension)
    if hitElement == localPlayer and matchingDimension and not isPedInVehicle(localPlayer) and activeIntData then
        if source == missionEntMarker and activeIntData.entTarg then
            triggerServerEvent("warpMysteryPlayer", localPlayer, activeIntData.entTarg.int or activeIntData.id or 0, activeIntData.entTarg.dim or 0, activeIntData.entTarg.x, activeIntData.entTarg.y, activeIntData.entTarg.z)
        elseif source == missionExitMarker and activeIntData.exitTarg then
            triggerServerEvent("warpMysteryPlayer", localPlayer, activeIntData.exitTarg.int or 0, activeIntData.exitTarg.dim or 0, activeIntData.exitTarg.x, activeIntData.exitTarg.y, activeIntData.exitTarg.z)
        end
    end
end)
...
addEventHandler("onMysteryComplete", root, function(reward)
    if activeIntData then
        local warpDest = activeIntData.exitTarg or activeIntData.entMark
        if warpDest then
            triggerServerEvent("warpMysteryPlayer", localPlayer, warpDest.int or 0, warpDest.dim or 0, warpDest.x, warpDest.y, warpDest.z)
        end
    elseif getElementInterior(localPlayer) ~= 0 or getElementDimension(localPlayer) ~= 0 then
        if mythX ~= 0 then
            triggerServerEvent("warpMysteryPlayer", localPlayer, 0, 0, mythX, mythY, mythZ)
        else
            triggerServerEvent("warpMysteryPlayer", localPlayer, 0, 0)
        end
    end
    ...
    setElementDimension(baseBlip, 0)
    setElementDimension(baseMarker, 0)
...
addEventHandler("onMysteryFailedClient", root, function(reason)
    if activeMission then
        ...
        if reason == "flee" then
            missionFailedReasonStr = "You ran, coward! Guess we need to hire better hunters next time."
        else
            missionFailedReasonStr = "Mission Failed! The mystery remains unsolved."
        end
...
local function handleEntityDamage(sourceElement, attacker, weapon, loss)
    if getElementData(sourceElement, "isMysteryPed") then
        ...
        -- Check if there are any specific Vulnerabilities (which restrict damage to specific types)
        ...
        if not isVulnRestricted then 
            canDamage = true
        else
            -- Process Restricted Vulnerabilities
            if string.find(vuln, "Explosion") and (weapon == 51 or weapon == 16 or (weapon >= 35 and weapon <= 36) or weapon == 39) then 
                canDamage = true
            end
            if string.find(vuln, "Fire") and (weapon == 37 or weapon == 16) then 
                canDamage = true
            end
            if string.find(vuln, "Water") and weapon == 53 then 
                canDamage = true
            end
            if string.find(vuln, "Bullets") and ((weapon >= 22 and weapon <= 34) or weapon == 38) then
                canDamage = true
            end
        end
        
        -- Process Specific Invulnerabilities (which block damage explicitly)
        if string.find(vuln, "Chainsaw") and weapon == 9 then
            canDamage = false
        end
        
        -- Block Minigun damage if explicitly invulnerable
        if string.find(vuln, "Minigun") and weapon == 38 then
            canDamage = false
        end
        
        -- Ensure entities are immune to their own Explosion ability
        if getElementData(sourceElement, "canExplosion") and weapon == 51 then
            canDamage = false
        end
        ...
        if canDamage then
            ...
            if not isPartyMember and isElement(targetPlayer) then
                ...
                for _, m in ipairs(members) do
                    if m == localPlayer then isPartyMember = true; break end
                end
            end
            ...
            elseif attacker == nil or (isElement(attacker) and getElementType(attacker) ~= "player") then
                -- For global/environmental damage, let the leader handle it to prevent multi-sync bugs
            ...
            if validAttacker then 
                triggerServerEvent("damageBossPed", sourceElement, loss) 
            end
        end
    end
end
...
addEventHandler("onClientPedDamage", root, function(attacker, weapon, bodypart, loss)
    handleEntityDamage(source, attacker, weapon, loss)
end)
...
-- ADDED/FIXED: Heavy Punch logic for players taking damage
addEventHandler("onClientPlayerDamage", localPlayer, function(attacker, weapon, bodypart, loss)
    if isElement(attacker) and getElementType(attacker) == "ped" and getElementData(attacker, "isMysteryPed") then
        if getElementData(attacker, "canHeavyPunch") and weapon == 0 then
            ...
            -- Massively increased horizontal velocity knockback
            local vx = math.cos(angle) * 15.0 
            local vy = math.sin(angle) * 15.0
            
            -- Use a timer to bypass MTA's damage animation velocity-lock
            setTimer(function()
                if isElement(localPlayer) and not isPedDead(localPlayer) then
                    setElementVelocity(localPlayer, vx, vy, 0.3) -- Lowered Z-lift so you don't fly up but backward
                end
            end, 50, 1)
            
            local extraDamage = 50 -- Adjusted based on feedback
            local armor = getPedArmor(localPlayer)
            if armor > extraDamage then
                setPedArmor(localPlayer, armor - extraDamage)
            else
...
addEventHandler("onClientRender", root, function()
    -- Lock time on render update if available
    if lockedTime then
        setTime(lockedTime[1], lockedTime[2])
    end
    ...
    if activeMission then
        ...
        for _, ent in ipairs(peds) do
            if isElement(ent) then
                ...
                local isAlive = false
                if eType == "Car" then isAlive = not isVehicleBlown(ent)
                else isAlive = not isPedDead(ent) end
    ...
    if activeMission then
        if isMysteryActive then
            ...
            if activeIntData then
                if activeIntData.entMark and currentInt == (activeIntData.entMark.int or 0) then
                    local dist = getDistanceBetweenPoints3D(px, py, pz, activeIntData.entMark.x, activeIntData.entMark.y, activeIntData.entMark.z)
                    if dist <= 300.0 then flee = false end
                end
                
                if activeIntData.entTarg and currentInt == (activeIntData.entTarg.int or activeIntData.id or 0) then
                    local dist = getDistanceBetweenPoints3D(px, py, pz, activeIntData.entTarg.x, activeIntData.entTarg.y, activeIntData.entTarg.z)
                    if dist <= 300.0 then flee = false end
                end
            end
            
            -- Check direct distance to the ped's original spawn if previous checks were not applicable
            if getDistanceBetweenPoints3D(px, py, pz, mythX, mythY, mythZ) <= 300.0 then
                flee = false
            end
        ...
        for i, entity in ipairs(entities) do
            if isElement(entity) then
                ...
                local isAlive = false
                if eType == "Car" then isAlive = not isVehicleBlown(entity)
                else isAlive = not isPedDead(entity) end
                ...
                if isAlive and not isElementFrozen(entity) then
                    ...
                    -- Gather all valid targets: Players AND Yakuza Guards
                    local potentialTargets = getElementsByType("player")
                    for _, ped in ipairs(getElementsByType("ped")) do
                        if getElementData(ped, "isYakuza") then
                    ...
                    for _, p in ipairs(potentialTargets) do
                        if not isPedDead(p) and getElementInterior(p) == getElementInterior(entity) and getElementDimension(p) == getElementDimension(entity) then
                            ...
                            if dist < minLoc then
                                minLoc = dist
                                closestPlayer = p  -- Variable kept as closestPlayer to maintain compatibility with downstream projectile logic
                                px, py, pz = tx, ty, tz
                            end
                        end
                    end
                        ...
                        if eType == "Car" then
                            ...
                            if isElement(driver) then
                                ...
                                if isReversing > getTickCount() then
                                    ...
                                    -- Check if stuck (Fixed: only triggers reverse if away from player)
                                    ...
                                    if speed < 0.05 and minLoc > 8.0 then
                                        ...
                                        if not stuckStart then
                                            setElementData(entity, "stuckStart", getTickCount())
                                        elseif getTickCount() - stuckStart > 3000 then -- Stuck for 3 seconds
                                            setElementData(entity, "isReversing", getTickCount() + 1500) -- Reverse for 1.5 seconds
                            ...
                            local ex, ey, ez = getElementPosition(entity)
                            local dist = minLoc
                            local rot = math.deg(math.atan2(py - ey, px - ex)) - 90
                            if rot < 0 then rot = rot + 360 end
                            setElementRotation(entity, 0, 0, rot, "default", true)
                            ...
                            elseif isThrowable or isRocket then
                                local attackDist = isRocket and 45.0 or 15.0
                                if dist <= attackDist then
                                    setPedControlState(entity, "forwards", false)
                                    setPedControlState(entity, "sprint", false)
...
local lastThrow = getElementData(entity, "lastThrowTime") or 0
                                    if getTickCount() - lastThrow > 3000 then
                                        ...
                                        -- FIXED: Client-side creation with guaranteed detonation timer for throwables
                                        setTimer(function()
                                            if isElement(entity) and not isPedDead(entity) then
                                                local ePx, ePy, ePz = getElementPosition(entity)
                                                
                                                -- Calculate throwing force based on distance so grenades don't overshoot (Rockets always use 1.0 max speed)
                                                local force = isRocket and 1.0 or math.max(0.2, math.min(1.0, dist / 20.0))
                                                
                                                -- Create visual projectile so it is thrown/shot
                                                local proj = createProjectile(entity, weapon, ePx, ePy, ePz + 0.5, force, closestPlayer)
                                                
                                                -- Force it to explode after 2.5 seconds ONLY for throwables (Rockets explode on impact natively)
                                                if proj and isThrowable then
                                                    local lastX, lastY, lastZ = getElementPosition(proj)
                                                    
                                                    -- Track the projectile's position in case the engine destroys it early
                                                    setTimer(function(pElement)
                                                        if isElement(pElement) then
                                                            lastX, lastY, lastZ = getElementPosition(pElement)
                                                        end
                                                    end, 50, 50, proj)
                                                    ...
                                                    setTimer(function(pElement)
                                                        ...
                                                        local explType = 0 -- Default Grenade/Satchel explosion
                                                        if weapon == 18 then explType = 4 end -- Molotov
                                                        if weapon == 39 then explType = 12 end -- Satchel
                                                        createExplosion(lastX, lastY, lastZ, explType)
                                                    end, 2500, 1, proj)
                                                end
